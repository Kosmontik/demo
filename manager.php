...
$filtered = [];
foreach($tovary as $row) {
    if($postav != '' && $row['name_postav'] != $postav) continue;
    if($search != '') {
        $text = $row['name_category'].' '.$row['name_typeTovar'].' '.$row['description_tovar'].' '.$row['name_proizvod'].' '.$row['name_postav'].' '.$row['ed_tovar'];
        if(mb_stripos($text, $search) === false) continue;
    }
    $filtered[] = $row;
}

if($sort == 'asc') usort($filtered, function($a, $b) { return $a['count_tovar'] - $b['count_tovar']; });
if($sort == 'desc') usort($filtered, function($a, $b) { return $b['count_tovar'] - $a['count_tovar']; });
